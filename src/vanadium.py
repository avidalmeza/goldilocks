...
# Set a, b, c, alpha, beta, gamma, Z for common vanadium 
a = 3.0299
b = 3.0299
c = 3.0299
alpha = 90.0
...
V_scatter_xs = float(sample.getMaterial().totalScatterXSection())
print(f'Total scattering cross-section: {V_scatter_xs}')

# Retrieve coherent scattering length
V_scatter_length = float(sample.getMaterial().cohScatterLength())
print(f'Coherent scattering length: {V_scatter_length}')

# Retrieve relative molecular mass
# Define molecular mass in g/mol/fu
V_molecular_mass = float(sample.getMaterial().relativeMolecularMass())
print(f'Relative molecular mass: {V_molecular_mass}')